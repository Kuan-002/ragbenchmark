...
_TWO_STAGE_THRESHOLD = 10000  # papers
_RRF_K               = 60
_BM25_WEIGHT         = 1.0
_DENSE_WEIGHT        = 0.6   # reduced per benchmark: Dense cross-corpus no advantage
_POOL_SIZE           = 50
_TOP_PAPERS          = 3     # two-stage: how many papers to retrieve
...
class LibraryIndex:
    ...
    def _search_single(self, query: str, effective_k: int,
                       llm_client=None, query_type: str = "factual") -> list[dict]:
        ...
        return self._postprocess(query, reranked, effective_k, llm_client, query_type)

    # ── Two-stage (> 500 papers) ────────────────────────────────────────────

    def _search_two_stage(self, query: str, effective_k: int,
                          llm_client=None, query_type: str = "factual") -> list[dict]:
        # Stage 1: paper-level BM25 (max-pool chunk scores per paper)
        bm25_scores  = self._bm25_scores_all(query)
        ...
        return self._postprocess(query, all_results, effective_k, llm_client, query_type)

    # ── Public search API ───────────────────────────────────────────────────

    def search(self, query: str, top_k: int = 5,
               llm_client=None, query_type: str = "factual") -> dict:
        ...
        if strategy == "two_stage":
            results = self._search_two_stage(query, effective_k, llm_client, query_type)
        else:
            results = self._search_single(query, effective_k, llm_client, query_type)
